...
const TABS = [
  ...
  { id: "Level", label: "লেভেল", icon: Layers, placeholder: "যেমন: সহজ" },
  {
    ...
    label: "স্পেশাল সার্চ",
    icon: Search,
    placeholder: "যেমন: হাফ ইয়ার্লি ২০২৬",
  },
];
...
export default function MetadataSetup() {
  ...
  const createMutation = useMutation({
    ...
    onError: (err) => {
      toast.error(
        err.response?.data?.error || err.message || "সংরক্ষণ করতে ব্যর্থ হয়েছে",
      );
    },
  });
  ...
  const editMutation = useMutation({
    ...
    onError: (err) => {
      toast.error(
        err.response?.data?.error || err.message || "আপডেট করতে ব্যর্থ হয়েছে",
      );
    },
  });
    ...
    onError: (err) => {
      toast.error(
        err.response?.data?.error ||
          err.message ||
          "স্ট্যাটাস পরিবর্তন করতে ব্যর্থ হয়েছে",
      );
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err) => {
      toast.error(
        err.response?.data?.error || err.message || "মুছে ফেলতে ব্যর্থ হয়েছে",
      );
    },
  });
  ...
  const handleSubmit = (e) => {
    ...
    if (isDuplicate) {
      ...
      toast.error(`"${trimmed}" নামের ${tabLabel} ইতিমধ্যে যুক্ত করা হয়েছে!`);
      return;
    }
  ...
  // Client-side pagination, sorting & search filtering logic
  ...
  const filteredMetadata = sortedMetadata.filter((item) => {
    if (!searchTerm.trim()) return true;
    const term = searchTerm.toLowerCase().trim();
    const nameMatch = item.name?.toLowerCase().includes(term);
    const shortNameMatch = item.shortName?.toLowerCase().includes(term);
    return nameMatch || shortNameMatch;
  });

  const totalItems = filteredMetadata.length;
          ...
          <div className="lg:col-span-2 bg-glass rounded-2xl border shadow-sm overflow-hidden">
            ...
            {isLoading ? (
              ...
              <div className="py-16 text-center text-slate-400 text-sm">
                ...
                &rdquo; নাম দিয়ে কোনো{" "}
                {TABS.find((t) => t.id === activeTab)?.label} খুঁজে পাওয়া যায়নি।
              </div>
            ) : (
