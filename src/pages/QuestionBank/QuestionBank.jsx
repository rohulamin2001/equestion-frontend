  ...
  return (
    <div className="space-y-6 pb-12 w-full font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight">প্রশ্নব্যাংক</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "মোট প্রশ্ন", count: totalCount, color: "from-indigo-500 to-indigo-600" },
          { label: "MCQ প্রশ্ন", count: mcqCount, color: "from-emerald-500 to-emerald-600" },
          { label: "সৃজনশীল (CQ)", count: creativeCount, color: "from-amber-500 to-amber-600" },
          { label: "সংক্ষিপ্ত ও অন্যান্য", count: otherCount, color: "from-violet-500 to-violet-600" },
        ].map((stat, i) => (
        ...
        <div className="space-y-4">
          {questions.map((q) => {
            ...
            return (
              <div
                key={q._id}
                className="bg-white p-6 rounded-2xl border border-slate-150 shadow-sm hover:shadow-md transition-all duration-200 flex flex-col space-y-4 relative cursor-pointer"
              ...
              >
                ...
                <div className="text-slate-800 font-serif leading-relaxed flex-1 pt-1 pointer-events-none">
                  ...
                  {q.category === "Creative" && q.creativeData && (
                    <div className="space-y-4">
                      {q.creativeData.stem && (
                        <div className="p-5 bg-slate-50 border border-slate-100 rounded-xl text-[14px] leading-relaxed text-slate-700 font-serif line-clamp-3">
                          {q.creativeData.stem}
                        </div>
                      )}
                  ...
                  {!["MCQ", "Creative"].includes(q.category) && q.generalData && (
                    <div className="space-y-3">
                      {q.generalData.stem && (
                        <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl text-sm italic font-serif leading-relaxed text-slate-700 line-clamp-2">
                          {q.generalData.stem}
                        </div>
                      )}
                ...
                <div
                  className="flex justify-between items-center border-t border-slate-100 pt-3 text-[11px] font-sans text-slate-400"
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      variant="outline"
                      onClick={() => setSelectedPreviewQuestion(q)}
                      className="border-slate-200 text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl h-8 px-3 text-xs flex items-center gap-1 font-bold cursor-pointer"
                    >
                    ...
                    {userCanManage && (
                      <>
                        ...
                        <Button
                          ...
                          className="border-slate-200 text-slate-600 hover:text-red-600 hover:bg-red-50 rounded-xl h-8 px-3 text-xs flex items-center gap-1 font-bold cursor-pointer"
                        >
      ...
      <Dialog open={!!selectedPreviewQuestion} onOpenChange={(open) => !open && setSelectedPreviewQuestion(null)}>
        <DialogContent className="max-w-2xl overflow-hidden p-0 rounded-2xl border border-slate-200 shadow-2xl bg-white flex flex-col">
          {selectedPreviewQuestion && (
            <>
              {/* Header */}
              <div className="bg-slate-950 text-white px-6 py-4 flex justify-between items-center">
                <h4 className="font-bold text-sm tracking-wide uppercase font-sans">NCTB Exam Question Sheet</h4>
                <div className="flex gap-2">
                  <span className="bg-indigo-500/25 text-indigo-300 text-[11px] font-bold px-3 py-1 rounded-full border border-indigo-500/20">
              ...
              {/* Exam Paper Sheet */}
              <div className="p-8 flex-1 bg-white text-slate-800 space-y-6 font-serif leading-relaxed max-h-[70vh] overflow-y-auto custom-scrollbar">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      {selectedPreviewQuestion.mcqData.explanation && (
                        <div className="mt-6 p-4 bg-slate-50 border border-slate-100 rounded-xl text-xs font-sans text-slate-500">
                ...
                {selectedPreviewQuestion.category === "Creative" && selectedPreviewQuestion.creativeData && (
                  <div className="space-y-5">
                    {selectedPreviewQuestion.creativeData.stem && (
                      <div className="p-5 bg-slate-50 border-l-4 border-l-indigo-500 border border-slate-100 rounded-r-xl text-[14px] leading-relaxed text-slate-700">
                        {selectedPreviewQuestion.creativeData.stem}
                      </div>
                    )}
                ...
                {!["MCQ", "Creative"].includes(selectedPreviewQuestion.category) && selectedPreviewQuestion.generalData && (
                  <div className="space-y-4">
                    ...
                    {selectedPreviewQuestion.generalData.suggestedAnswer && (
                      <div className="p-4 bg-indigo-50/20 border border-indigo-50/50 rounded-xl text-xs font-sans text-indigo-900/80">
              ...
              {/* Footer Actions */}
              <div className="bg-slate-50 border-t px-6 py-4 flex flex-col sm:flex-row gap-3 items-center justify-between">
                ...
                <div className="flex gap-2 w-full sm:w-auto justify-end">
                  {canManageQuestion(selectedPreviewQuestion) && (
                    <Button
                      ...
                      className="border-slate-200 text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl font-semibold text-xs h-9 px-4 cursor-pointer"
                    >
                  ...
                  <Button
                    ...
                    className="bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-semibold text-xs h-9 px-4 cursor-pointer"
                  >
                    বন্ধ করুন
                  </Button>
                </div>
